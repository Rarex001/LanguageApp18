  ...
  try {
    ...
    if (tokens.length > 1) {
      await Promise.all(tokens.map(async (token) => {
        ...
        wordConfidence[key] = dictPos ? 1 : (posConfidence(clean, guessedPos) ? 0.96 : 0.5)
        try {
      ...
      wordConfidence[clean] = dictPos ? 1 : (posConfidence(clean, guessedPos) ? 0.96 : 0.5)
    }
    return NextResponse.json({ translation, wordTranslations, wordPos, wordConfidence, original: word })
